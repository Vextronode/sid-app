...
export function DesktopNavbar() {
  ...
  if (isLoading) {
    return <nav className="sid-desktop-navbar-loading" />
  }
  ...
  return (
    <nav className="sid-desktop-navbar">
      ...
      <div className="sid-desktop-navbar-logo">LOGO</div>
        ...
        {NAV_LINKS.map((link) => {
          ...
          return (
            <Link
              key={link.name}
              to={link.href}
              className={`sid-desktop-navbar-link ${isActive ? 'active' : ''}`}
            >
              {link.name}
            </Link>
          )
      ...
      <div className="sid-desktop-navbar-actions">
        {user ? (
          <div className="sid-desktop-navbar-user">
            ...
            <button onClick={handleLogout} className="sid-desktop-navbar-logout">
              <LogOut size={14} />
              <span>Keluar</span>
            </button>
          </div>
        ) : (
          <Link to="/loginpage" className="sid-desktop-navbar-login">
            Masuk
          </Link>
        )}
      </div>
