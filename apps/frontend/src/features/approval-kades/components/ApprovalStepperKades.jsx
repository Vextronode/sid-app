// ==========================================
// ApprovalStepperKades.jsx
// Sama seperti ApprovalStepperRT, cuma dipakai buat monitoring.
// ==========================================
...
export default function ApprovalStepperKades({ surat }) {
  ...
  return (
    <div className="flex items-center justify-center gap-1 mb-6 flex-wrap">
      {STEPS.map((label, index) => {
        ...
        const isDone = index === 0 || (index === 1 && (state === 'current_rw' || state === 'done_rw')) || (index === 2 && state === 'done_rw');
        const isCurrent = (index === 2 && state === 'current_rw') || (index === step && state === 'current');

        if (isRejectedHere) {
          circle = <div className="w-9 h-9 rounded-full bg-red-500 text-white flex items-center justify-center"><X size={18} /></div>;
          statusText = 'Ditolak'; labelColor = 'text-red-500';
        } else if (isDone) {
          circle = <div className="w-9 h-9 rounded-full bg-green-500 text-white flex items-center justify-center"><Check size={18} /></div>;
          statusText = 'Disetujui'; labelColor = 'text-green-600';
        } else if (isCurrent) {
          ...
          statusText = 'Menunggu'; labelColor = 'text-green-500';
        } else {
