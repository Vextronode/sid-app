...
import { useSuratList } from '@/features/approval-kasi/hooks/useSuratListkasi';
import { useApprovalAction } from '@/features/approval-kasi/hooks/useApprovalActionkasi';
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-medium text-gray-800">Semua permohonan surat (Kasi)</h2>
        <button className="border border-green-500 text-green-600 rounded-full px-4 py-1.5 text-sm">Tambah Surat</button>
      </div>
      <SearchFilterBarkasi onSearch={setSearch} onFilterJenis={setFilterJenis} onFilterStatus={setFilterStatus} selectedStatus={filterStatus} />
      <SuratTablekasi
        data={paginatedData}
      ...
      <div className="flex items-center justify-between mt-4">
        ...
        <Paginationkasi currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} />
      </div>
      <SuratDetailModalkasi suratId={selectedId} onClose={() => { setSelectedId(null); setIsReadOnly(false); }} onApprove={handleApprove} onReject={handleReject} readOnly={isReadOnly} />
    </div>
  );
}