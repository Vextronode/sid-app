...
export default function TestRevisionFlowPage() {
  ...
  const loadLetters = async () => {
    ...
    try {
      ...
      setMessage('Gagal memuat daftar surat');
    } finally {
      setLoading(false);
    }
  };

  const handleApprove = async (selectedAction) => {
  ...
  const handleResubmit = async () => {
    if (!selectedLetter || !resubmitData.purpose) {
      setMessage('Mohon isi keperluan');
      return;
    }
    ...
    try {
      await api.patch(
        `/api/letters/${selectedLetter.id}/resubmit`,
        resubmitData
      );

      setMessage('Surat berhasil dikirim ulang');
    ...
    <div className="p-8 bg-gray-50 min-h-screen">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">
          Testing Alur Revisi Surat
        </h1>

        <p className="text-gray-600 mb-6">
          Halaman khusus untuk testing flow:
          create → approve RT → proses kantor desa → minta revisi →
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
          <div className="flex gap-4 mb-6 flex-wrap">
            ...
            <button
              ...
              disabled={
                selectedLetter?.status !== 'waiting_revision_warga'
              }
              className={`px-4 py-2 rounded-lg font-medium transition ${
                testStep === 'resubmit' &&
                selectedLetter?.status === 'waiting_revision_warga'
          ...
          {testStep === 'action' && selectedLetter && (
            <div className="space-y-4">
              <h2 className="text-xl font-bold text-gray-800">
                Aksi untuk Surat #{selectedLetter.id}
              </h2>

              <div className="bg-gray-50 p-4 rounded-lg">
                <p className="text-sm text-gray-600">
                  <strong>Jenis:</strong>{' '}
                  {selectedLetter.letter_type?.name ?? '-'}
                </p>

                <p className="text-sm text-gray-600">
                  <strong>Pemohon:</strong>{' '}
                  {selectedLetter.applicant_name ?? '-'}
                </p>
              ...
              {user?.role === 'rw' ? (
                <div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
                  <p className="text-sm text-blue-800">
                    ...
                    RW tidak dapat menyetujui, menolak, atau
                ...
                <>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    {(user?.role === 'kasi_pelayanan' ||
                      user?.role === 'kaur_tu_umum' ||
                      user?.role === 'petugas_desa') && (
                      <button
                        type="button"
                        onClick={() =>
                          handleApprove('needs_revision')
                        }
                        disabled={loading}
          ...
          {testStep === 'resubmit' &&
            selectedLetter?.status ===
              ...
              <div className="space-y-4">
                <h2 className="text-xl font-bold text-gray-800">
                  Resubmit Revisi - Surat #{selectedLetter.id}
                </h2>

                <div className="bg-amber-50 border border-amber-200 p-4 rounded-lg">
                  <p className="text-sm text-amber-800">
                    Surat ini sedang menunggu revisi dari
                    warga. Isi form di bawah untuk melakukan
                    resubmit.
                  </p>
                </div>
          ...
          <div className="mt-8 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <ul className="text-sm text-blue-800 space-y-1 list-disc pl-5">
              <li>
                Jika role RT: bisa approve/reject dari
                status <strong>pending</strong>.
              </li>

              <li>
                Jika role RW: hanya dapat memantau surat
                dan tidak memiliki kewenangan keputusan.
              </li>

              <li>
                Jika role Kasi/Kaur/Operator: bisa
                approve/reject/minta revisi setelah RT
                menyetujui dengan status{' '}
                <strong>rt_approved</strong>.
              </li>

              <li>
                Jika surat berstatus{' '}
                <strong>waiting_revision_warga</strong>,
                warga dapat melakukan resubmit.
              </li>

              <li>
                Setelah resubmit, surat kembali ke alur
                pemrosesan kantor desa sesuai status yang
                ditentukan backend.
              </li>
            </ul>
          </div>
